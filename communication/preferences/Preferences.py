...
class Preferences:
    ...
    def most_preferred(self, item_list):
        ...
        """
        scores = [(i, i.get_score(self)) for i in item_list]
        sorted_scores = sorted(scores, key=lambda x: x[1], reverse=True)
        max_score = sorted_scores[0][1]
        preferred_items = [i[0] for i in sorted_scores if i[1] == max_score]
        return random.choice(preferred_items)
    ...
    def is_item_among_top_10_percent(self, item, item_list):
        ...
        """
        scores = [i.get_score(self) for i in item_list]
        sorted_scores = sorted(scores, reverse=True)
        item_score = item.get_score(self)
        index = sorted_scores.index(item_score)
        return index <= len(sorted_scores) * 0.1
